...
public class TradeSceneManager : MonoBehaviour
{
    ...
    public List<CardInfo> tradingCards;

    //Uncomment this while implementing in Crocuta
    //public Inventory playerInventory; // Reference to the Inventory ScriptableObject
    //public EncounterInfo encounterInfo; // Reference to the EncounterInfo ScriptableObject


    public static TradeSceneManager Instance { get; private set; }
    ...
    private void Awake()
    {
        ...
        // Fix me: Uncomment this while implementing in Crocuta
        //playercardData = playerInventory.startItems;
        //tradeCards = encounterInfo.encounterCards;

        AddStartingCards();

    }

    public List<CardInfo> GetTradeCards()
    {
        return tradingCards;
    }
    ...
    {
        // Instantiate Card
        GameObject card = generateCard(info, handTransform, tag = "HandCard");
    ...
    public GameObject generateCard(CardInfo info, Transform parentTransform, string tag)
    {
    ...
    public void swapCards(CardInfo infoHandCard, CardInfo infoBoardCard)
    {
        // Find the index of the hand card that matches infoHandCard
        int handCardIndex = cardData.FindIndex(card => card.Equals(infoHandCard));

        // If a matching hand card is found
        if (handCardIndex != -1)
        {
            // Replace the hand card with the board card at the found index
            cardData[handCardIndex] = infoBoardCard;

            print("Cards swapped successfully!");
        }
        else
        {
            Debug.LogError("Hand card not found!");
        }
    }
